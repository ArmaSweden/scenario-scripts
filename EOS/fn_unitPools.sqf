...
Description:
Hanterar pooler av enheter baserat på fraktion och typ.

Variables:
- Enhetspooler för olika fraktioner och typer.

Remarks:
Skriptet definierar och hanterar enhetspooler för olika fraktioner och typer, samt returnerar en array med skapade enheter.

Parameter(s):
- _faction: Fraktion för enheterna.
- _type: Typ av enhet eller grupp som ska skapas.

Important Notes:
- Enhetspooler måste vara korrekt definierade för varje fraktion och typ.
- Skriptet väljer slumpmässigt enheter från poolen baserat på angiven fraktion och typ.

Potential Errors:
- Om enhetspoolen inte är korrekt definierad kan det leda till fel vid skapande av enheter.

Returns:
- Returnerar en array med skapade enheter.
*/


private _timeStamp = diag_tickTime;

diag_log format ["[fn__unitPool.sqf] starts at %1", _timestamp];

// Definiera variabler för enhetspooler
private [
"_InfPool",
"_MotPool",
"_ACHPool",
...
private _faction = (_this select 0); // Vald fraktion
private _type = (_this select 1); // Typ av enhet eller grupp som ska skapas
private _tempArray = []; // Temporär array för att lagra skapade enheter
...
////////////////////////////////////////////////////////////////////////////////////////
// Skapa enheter baserat på vald typ
if (_type==0) then {
		// Skapa infanteri, väljer slumpmässigt från infanteripoolen
		for "_i" from 0 to 5 do{
		_unit=_InfPool select (floor(random(count _InfPool)));
		_tempArray set [count _tempArray,_unit];};
						};

// Skapa dykare, enbart om typen är angiven för dykare
if (_type==1) then {_tempArray=_diverPool};
	
// Typ 2: Skapa pansarfordon och tilldela besättning				
// CREATE ARMOUR & CREW			
if (_type==2) then {
...
// Typ 3: Skapa attackhelikopter och tilldela pilot
...
// Typ 4: Skapa transportflyg och tilldela besättning
...
// Typ 5: Skapa statiska försvar och tilldela besättning						
// CREATE STATIC & CREW						
if (_type==5) then {
...
// Typ 6: Skapa och välj UAV
if (_type==6) then {_tempArray=_uavPool select (floor(random(count _uavPool)));};

// Typ 7: Skapa transportfordon och tilldela besättning
...
// Typ 8: Skapa båtar och tilldela dykarbesättning
...
// Typ 9 och 10: Skapa last med infanteri eller dykare, respektive				
// CREATE CARGO
if (_type==9) then {
...
// Avslutar med att returnera den temporära arrayen med alla skapade enheter
