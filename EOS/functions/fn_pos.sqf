...
diag_log format ["[fn_pos.sqf] starts at %1", _timestamp];

// Funktion för att beräkna en ny position baserat på ursprung, riktning och avstånd
private "_getpos";
_getpos = {
  private ["_origo","_dir","_dist"];
  _origo = _this select 0; // Startpunkt
  _dir = _this select 1; // Riktning i grader eller ett intervall av grader
  _dist = _this select 2; // Avstånd eller ett intervall av avstånd
  // Om riktningen är ett intervall, välj ett slumpmässigt värde inom detta intervall
  ...
  // Om avståndet är ett intervall, välj ett slumpmässigt värde inom detta intervall
  if (typename _dist == typename []) then {
    _dist = ((_dist select 0) + random((_dist select 1)-(_dist select 0)));
  };
  // Beräknar den nya positionen baserat på startpunkt, riktning och avstånd
  [((_origo select 0) + (_dist * sin _dir)),((_origo select 1) + (_dist * cos _dir)),0];
};

// Huvudlogik för att hantera positionering baserat på område och egenskaper
private "_water";
if (typename (_this select 0) == typename "") then {
  private ["_pos","_area","_cp","_cx","_cy","_as","_rx","_ry","_ad","_cd","_sd","_xo","_yo","_loop"];
  _area = _this select 0; // Områdesmarkör
  // Väljer om vattenhantering ska tillämpas eller inte
  if (count _this > 1) then {_water = _this select 1} else {_water = false};
  _cp = getMarkerPos _area; // Centrumposition för områdesmarkören
  _cx = abs(_cp select 0); // Absoluta x-värdet för centrumpositionen
  _cy = abs(_cp select 1); // Absoluta y-värdet för centrumpositionen
  _as = getMarkerSize _area; // Storlek på området
  _rx = _as select 0; // Radien i x-led
  _ry = _as select 1; // Radien i y-led
  _ad = (markerDir _area) * -1; // Riktningen för områdesmarkören negativt
  _cd = cos _ad; // Cosinus för riktningen
  _sd = sin _ad; // Sinus för riktningen
  _loop = true; // Loop-kontrollvariabel
  // Loop för att hitta en position inom området som uppfyller villkoren (på land eller i vatten)
  while {_loop} do {
    _tx = (random (_rx*2))-_rx; // Temporärt x-värde inom områdets bredd
    _ty = (random (_ry*2))-_ry; // Temporärt y-värde inom områdets höjd
    _xo = if (_ad!=0) then {_cx+ (_cd*_tx - _sd*_ty)} else {_cx+_tx}; // Beräknad x-position med rotation om tillämpbart
    _yo = if (_ad!=0) then {_cy+ (_sd*_tx + _cd*_ty)} else {_cy+_ty}; // Beräknad y-position med rotation om tillämpbart
    _pos = [_xo,_yo,0]; // Slutlig position
    // Kontrollerar om positionen är i vatten om så krävs, och avslutar loopen om positionen är giltig
    if (_water) then {
      _loop = false;
    } else {
  ...
  _pos // Returnerar den funna positionen
} else {
  // Deklarerar och initierar variabler som används i denna del av skriptet
  private ["_origo","_dir","_dist","_pos","_loop","_watersolution"];
  _origo = _this select 0; // Ursprungspunkt
  _dir = _this select 1; // Riktning
  _dist = _this select 2; // Avstånd
  // Hanterar om vattenkontroll är aktiverad
  if (count _this > 3) then {_water = _this select 3} else {_water = false};
  // Väljer vilken vattenlösning som ska användas baserat på input
  if (count _this > 4) then {_watersolution = _this select 4} else {_watersolution = 1};
  // Beräknar startpositionen
  _pos = [_origo,_dir,_dist] call _getpos;

  // Kontrollerar om vattenkontroll är deaktiverad
  if (!_water) then {
    private ["_d","_l","_p"];
    _l = true; // Loopkontroll
    // Hanterar olika metoder för att anpassa position baserat på vattenkontroll
    switch _watersolution do {
      case 0: { // Om positionen är i vatten, töm _pos
        if (surfaceIsWater [_pos select 0,_pos select 1]) then {
          _pos = +[];
        };
      };
      case 1: { // Expandera sökradien stegvis tills en torr plats hittas
        _d = 10; _l = true;
        while {_l} do {
          for "_i" from 0 to 350 do { // Söker i en cirkel runt ursprungspunkten
            _p = [_pos,_i,_d] call _getpos;
            if (!surfaceIsWater [_p select 0,_p select 1]) exitwith {_l = false};
          };
          _d = _d + 10; // Öka sökradien
        };
        _pos = _p; // Uppdatera positionen till den torra platsen
      };
      case 2: { // Minska avståndet tills en torr plats hittas
      ...
      case 3: { // Öka avståndet tills en torr plats hittas
        _d = _pos distance _origo;
        while {_d = _d + 10; _l} do {
          _pos = [_pos,_dir,_d] call _getpos;
          if (!surfaceIsWater [_pos select 0,_pos select 1]) then {_l = false};
      ...
      case 4: { // Sök bakåt från angiven riktning
        if (typename _dir == typename []) then {
          _d = _dir select 0;
          _dir = _dir select 0;
        } else {
      ...
      case 5: { // Sök framåt från angiven riktning
        if (typename _dir == typename []) then {
          _d = _dir select 1;
          _dir = _dir select 1;
        } else {
  ...
  _pos // Returnerar den slutliga positionen
};